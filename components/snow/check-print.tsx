  ...
  return (
    <ModalShell id="snow-check" onClose={onClose} title="눈 오기 전 점검 후보 · 결재용 한 장" sub={`광진 제설 상황판 · 자원 기준일 ${asof} · 빌드 ${data.meta.built}`} size="xl" zIndex={2400}>
      ...
      <p className="mt-3 text-[12.5px] leading-snug text-[var(--cp-text-dim)]">
        번호가 우선순위입니다(구 소관 행동 가능 › 시 요청 › 동 단위 › 학교). 부서·규모는 공개 데이터에 있는 것만 적었고 학교 소관은 데이터가 없어 내부 확인입니다. 비용은 자재·열선 단가 자료가 없어 칸을 두지 않았습니다. 거리 기준({data.gaps.heatNearM}m 열선·{data.gaps.materialNearM}m 자재·{data.gaps.schoolNearM}m 학교)은 이 화면의 가정입니다. 조치 여부와 순서는 담당 부서가 정합니다.
      </p>
