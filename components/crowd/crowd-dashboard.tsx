...
import { ArrowLeft, LoaderCircle, MapPin, RefreshCw, Search, X } from "lucide-react"
...
export default function CrowdDashboard() {
  ...
  return (
    <div className="crowd-page flex h-dvh flex-col bg-[#0b0f14] text-slate-300">
      {/* ── 헤더 */}
      <header className="flex h-14 shrink-0 items-center justify-between gap-3 border-b border-white/10 px-4 md:px-5">
        <div className="flex min-w-0 items-baseline gap-3">
          <h1 className="shrink-0 text-xl text-white [font-family:Joseon100Years,serif] md:text-2xl">
            서울 인파실록
          </h1>
        ...
        <div className="flex shrink-0 items-center gap-3 md:gap-4">
          <div className="hidden items-center gap-3 md:flex">
            {LEVEL_ORDER.map((level) => (
              <div key={level} className="flex items-center gap-1.5">
                ...
                <span className="font-mono text-[12px] tabular-nums text-slate-200">
                  {levelCounts[level] ?? 0}
                </span>
              </div>
            ))}
          ...
          <div className="flex items-center gap-1.5 border-l border-white/10 pl-3 md:pl-4">
            {updatedAt && (
          ...
          <Link
            href="/"
            className="hidden border-l border-white/10 pl-3 text-[11px] text-slate-500 transition-colors hover:text-white sm:block md:pl-4"
          >
            표준주소실록 ↗
          </Link>
        </div>
      ...
      <div className="flex min-h-0 flex-1 flex-col md:flex-row">
        ...
        >
          ...
          <div className="absolute bottom-2 left-2 z-[1000] flex items-center gap-2 rounded-full border border-white/10 bg-[#0b0f14]/85 px-2.5 py-1 backdrop-blur-sm md:hidden">
            {LEVEL_ORDER.map((level) => (
          ...
          {loading && (
            <div className="absolute inset-0 z-[1050] flex items-center justify-center bg-[#0b0f14]/80">
              <LoaderCircle className="h-6 w-6 animate-spin text-slate-400" />
            </div>
          )}
        </div>

        <aside className="flex min-h-0 flex-1 flex-col border-t border-white/10 md:w-[420px] md:flex-none md:border-l md:border-t-0">
          {/* 검색 */}
          <div className="shrink-0 border-b border-white/10 p-3">
            <div className="flex items-center gap-2 rounded-md border border-white/15 bg-white/[0.03] px-3 focus-within:border-white/30">
              ...
              <input
                ...
                className="h-10 w-full bg-transparent text-[16px] text-slate-200 placeholder:text-slate-600 focus:outline-none md:h-9 md:text-[13px]"
              />
              {query && (
                <button
                  onClick={() => {
            ...
            {noSpotMatch && !addressPin && (
              <button
                ...
                className="mt-2 flex w-full items-center justify-center gap-1.5 rounded-md border border-white/15 bg-white/[0.05] py-2 text-[12px] text-slate-300 transition-colors hover:bg-white/10 disabled:opacity-50"
              >
                {addressLoading ? (
          ...
          {selectedName ? (
            <div className="flex min-h-0 flex-1 flex-col">
              <button
                ...
                className="flex shrink-0 items-center gap-1.5 border-b border-white/10 px-4 py-2.5 text-[12px] text-slate-400 transition-colors hover:text-white"
              >
            ...
            <div className="flex min-h-0 flex-1 flex-col">
              <div className="flex shrink-0 items-start justify-between gap-2 border-b border-white/10 px-4 py-3">
                <div className="min-w-0">
                  <p className="text-[10px] uppercase tracking-wider text-slate-500">검색한 주소</p>
                  <p className="mt-0.5 truncate text-[13px] font-medium text-white">{addressPin.label}</p>
                </div>
                <button
                  onClick={() => setAddressPin(null)}
                  className="shrink-0 rounded p-1 text-slate-500 hover:bg-white/5 hover:text-white"
                  aria-label="주소 검색 해제"
                >
                  <X className="h-4 w-4" />
                </button>
              ...
              <div className="min-h-0 flex-1 overflow-y-auto">
                <p className="px-4 pb-1 pt-3 text-[10px] uppercase tracking-wider text-slate-500">
                  근처 명소 인파 상황
                </p>
                <ul>
                  {nearest.map(({ spot, km }, i) => (
                    <li key={spot.name}>
                      <button
                        ...
                        className="group flex w-full items-center gap-3 border-b border-white/5 px-4 py-3 text-left transition-colors hover:bg-white/[0.04]"
                      >
                        <span className="w-4 shrink-0 font-mono text-[11px] tabular-nums text-slate-600">
                          {i + 1}
                        </span>
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-[13px] text-slate-200 group-hover:text-white">
                            {spot.name}
                          </p>
            ...
            <div className="flex min-h-0 flex-1 flex-col">
                  ...
                  {(
                    ...
                    <button
                      ...
                      className={`px-1.5 py-1 text-[11px] transition-colors ${
                        sort === mode ? "text-white underline underline-offset-4" : "text-slate-600 hover:text-slate-400"
                      }`}
                    >
                      {label}
                    </button>
              ...
              <div className="min-h-0 flex-1 overflow-y-auto">
                ...
                <ul>
                  {filtered.map((spot, i) => (
                    <li key={spot.name}>
                      <button
                        onClick={() => selectSpot(spot.name)}
                        className="group flex w-full items-center gap-3 border-b border-white/5 px-4 py-2.5 text-left transition-colors hover:bg-white/[0.04]"
                      >
                        <span className="w-6 shrink-0 font-mono text-[11px] tabular-nums text-slate-600">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-[11px] text-slate-500">{spot.category}</p>
                        </div>
                        <span
                          className="shrink-0 rounded-full px-2 py-0.5 text-[11px] font-semibold"
                          style={{ color: spot.color, background: `${spot.color}1a` }}
                ...
                {!loading && !error && filtered.length === 0 && !noSpotMatch && (
                  <p className="p-6 text-center text-[12px] text-slate-500">조건에 맞는 명소가 없습니다.</p>
                )}
                {noSpotMatch && (
                  <p className="p-6 text-center text-[12px] text-slate-500">
          ...
          <footer className="shrink-0 border-t border-white/10 px-4 py-2 pb-[max(0.5rem,env(safe-area-inset-bottom))]">
