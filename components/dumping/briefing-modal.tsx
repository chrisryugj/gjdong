  ...
  return (
      ...
      footer={
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 rounded-lg bg-(--dump-accent) py-2 text-[15.5px] font-semibold text-white hover:bg-[#0a5449]"
          >
            인쇄 / PDF 저장
          </button>
          <button
    ...
    >
      ...
      <section className="mt-4">
        ...
        <ul className="mt-1 flex flex-col gap-1">
          {recs.map((r) => (
            <li key={r.t} className="rounded-lg bg-(--dump-accent)/8 px-2.5 py-1.5 text-[14.5px] leading-snug">
              <span className="font-medium text-(--dump-accent-ink)">{r.t}</span>
      ...
      <p className="mt-3 text-[13px] leading-relaxed text-[var(--cp-text-faint)]">
        ...
        {graph ? collinearRange(graph) : "0.85~0.97"}) 어느 하나를 원인으로 지목하지 못합니다. 개입은 실행 전에 조치 대장에 사전등록하고 비교 대상을 정한
        뒤 평가해 주세요.
      </p>
    </ModalShell>
  )
