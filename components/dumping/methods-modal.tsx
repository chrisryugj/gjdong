...
const methods = (data: DumpingMapData, graph: OntoGraph | null): Method[] => {
  ...
  return [
    {
      name: "100m 격자 결합",
      easy: `구 전체를 100m 바둑판(회귀 표본 ${n(sz.gridN)}칸)으로 나누고 민원·과태료·건축물대장·도로·생활인구를 모두 같은 칸에 결합했습니다. 서로 다른 자료를 한 지도에서 비교할 수 있게 만드는 기초 작업입니다.`,
      here: `자료마다 칸에 넣는 방식이 다릅니다. 민원 ${n(s.complaints)}건·과태료 ${n(data.decision.fines.totalN)}건은 건별 주소를 좌표로 바꿔 점이 속한 칸에 배정했습니다. 건축물대장 ${Number.isFinite(sz.ledgerRows) ? n(sz.ledgerRows) : "미산출"}동은 대지 지번 좌표로, 도로·건물 형태는 OSM 선·면을 칸 경계로 나눠 넣었습니다. 서울시 생활인구는 250m 격자를 면적 비례로 나누고 SGIS 상주인구는 100m 격자를 그대로 결합했습니다. 동별 천명당 지표는 주민등록 인구로 따로 계산합니다.`,
      caution: `"100m로 나눠서 그런 결과가 나온 것 아니냐"는 물음에는 격자를 200m로 합쳐 다시 적합한 결과로 답합니다(격자 검증 카드). 판정 유지 ${data.decision.regressionV2 ? `${Object.values(data.decision.regressionV2.gridSensitivity.v2).filter(Boolean).length}/${Object.keys(data.decision.regressionV2.gridSensitivity.v2).length}`: "미산출"} 변수. 격자는 통계청 EPSG:5179 정렬이라 SGIS 인구격자·서울시 250m 격자와 좌표로 바로 이어집니다.`,
    },
    {
      name: "다중회귀 분석 (표준화 β)",
    ...
    },
    {
      ...
      easy: "CCTV를 설치한 곳과 아직 설치하지 않은 곳을 전후로 비교해 효과를 재는 방법입니다. 함정이 하나 있습니다. 원래 많이 발생하던 곳은 아무것도 하지 않아도 저절로 줄어드는 경향(평균회귀)이 있어서 비교를 잘못 짜면 없는 효과가 있어 보입니다.",
      here: "초기 분석은 감소 효과가 있다고 봤지만 비교 대상에 같은 조건을 적용해 다시 분석하니 비교 대상도 똑같이 줄었습니다. 감소분이 평균회귀로 설명돼 선택 규칙과 대조군 정의에 민감한 효과 주장을 철회했습니다. 설치 전후를 월 단위로 펼쳐 본 이벤트 스터디(관측 22,247행)에서도 유의한 시점은 없었습니다.",
    ...
    },
    {
      ...
      caution: `배율은 ${cg.basis}한 값입니다. 과태료의 ${100 - cg.patrolSharePct}%는 신고를 받고 단속한 것이라 신고와 독립인 실측으로 볼 수 없습니다. ${finesCensorNote(data)}.`,
    },
    {
      name: "핫스팟 점수와 백테스트",
      ...
      caution: bt.baselines
        ? "최근성 가중 점수는 단순 누적 빈도 목록과 동급이며 우열을 확정하지 않습니다. 이 점수식이 실무 목록보다 낫다는 주장은 하지 않고, 매 분기 같은 규칙으로 뽑아 사후 채점한다는 점만 강조합니다."
        : undefined,
    },
    {
      name: "홀트윈터스 수요 전망",
