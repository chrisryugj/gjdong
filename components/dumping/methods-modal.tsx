...
const METHODS: Method[] = [
  {
    name: "100m 격자 결합",
    easy: "구 전체를 100m 바둑판(1,062칸)으로 나누고, 민원·과태료·건축물대장·인구를 모두 같은 칸 위에 얹었습니다. 서로 다른 장부를 한 지도에서 견줘 볼 수 있게 만드는 밑작업입니다.",
    here: "민원 3,462건과 과태료 3,247건을 주소로 좌표화해 격자에 배정했고, 건축물대장 24,520동으로 칸마다 무관리 주거단위 수를 셌습니다.",
  },
  {
    name: "다중회귀 분석 (표준화 β)",
    easy: "여러 요인이 섞여 있을 때 각 요인의 몫을 갈라내는 계산입니다. \"인구가 많아서인가, 관리가 없어서인가\"를 한꺼번에 넣고 따로 재는 셈이고, β는 그 몫의 크기입니다.",
    here: "격자 1,062칸에서 과태료 건수를 종속변수로 놓고 재 보니, 관리주체 없는 주거 밀도가 β +0.312로 가장 컸고 공동주택 세대수는 무효(p=0.708), 골목 비율은 오히려 음수(−0.222)였습니다.",
    caution: "계산 방법을 네 가지(기본 OLS, 이분산 보정, 군집 보정, 음이항)로 바꿔 가며 전부 같은 결론일 때만 채택했습니다. 그래도 이것은 조건부 연관이지 인과를 증명한 것은 아닙니다.",
  },
  {
    name: "이중차분(DID)과 이벤트 스터디",
    easy: "CCTV를 설치한 곳과 아직 설치하지 않은 곳을 전후로 견줘 효과를 재는 방법입니다. 여기에는 중요한 함정이 하나 있습니다. 원래 많이 발생하던 곳은 아무것도 하지 않아도 저절로 줄어드는 경향(평균회귀)이 있어서, 비교를 잘못 짜면 없는 효과가 있어 보입니다.",
    here: "초기 분석은 감소 효과가 있다고 봤지만, 비교 대상에 같은 조건을 걸어 다시 재니 그쪽도 똑같이 줄었습니다. 감소분이 전부 평균회귀였던 것이라 주장을 철회했습니다. 설치 전후를 월 단위로 펼쳐 본 이벤트 스터디(관측 22,247행)에서도 유의한 시점은 없었습니다.",
    caution: "이 철회 경험이 조치 대장(개입 사전등록) 원칙의 근거입니다. 효과 평가는 실행 전에 설계부터 등록합니다.",
  },
  {
    name: "신고 채널 분해",
    easy: "민원이 늘었다고 해서 발생이 는 것은 아닙니다. 신고 창구(앱·120·직접)별로 쪼개 보면 무엇이 늘었는지가 갈립니다.",
    here: "민원 2.10배 증가를 쪼개 보니 앱만 2.97배였고 120·직접은 1.10배였습니다. 신고 성향과 무관한 과태료도 1.1배 수준이라, 늘어난 몫은 대부분 앱 보급 효과로 봅니다.",
  },
  {
    name: "핫스팟 점수와 백테스트",
    easy: "최근에 생긴 일일수록 무겁게 쳐서(90일이 지나면 절반 가중) 격자마다 점수를 매기고, 점수가 높은 지역을 다음 분기 관리 대상으로 뽑습니다. 믿을 만한지는 과거로 돌아가 확인합니다. 작년 이맘때 이 방법으로 뽑았다면 실제로 맞았을지를 8개 분기에 걸쳐 되풀이해 채점했습니다.",
    here: "상위 20곳 가운데 평균 65%에서 다음 분기 실제 발생이 있었고, 구 전체 발생의 11.5%가 이 20곳 안에서 일어났습니다(아무 곳이나 20곳을 찍으면 2.8%입니다).",
  },
  {
    name: "홀트윈터스 수요 전망",
    easy: "월별 접수의 수준과 추세, 계절 반복(여름에 많고 겨울에 적은 흐름)을 학습해 다음 달을 내다보는 시계열 모형입니다.",
    here: "직전 8개월을 한 달씩 가려 놓고 맞혀 보는 검증에서 평균 오차가 18%였습니다. 인력과 순찰 배치를 위한 행정수요 전망으로만 씁니다.",
    caution: "신고 접수량 전망이지 발생량 예측이 아닙니다.",
  },
  {
    name: "온톨로지 (지식그래프)",
    easy: "데이터셋·증거·주장·지표·대책을 점으로 놓고, 그 사이 관계(뒷받침한다, 겨냥한다)를 선으로 이은 지식 지도입니다. 표로는 던질 수 없는 질문, 이를테면 \"연관 요인 가운데 대책이 없는 것은?\" 같은 물음을 기계적으로 물을 수 있습니다.",
    here: "69개 지식과 95개 연결로 엮었습니다. 청년·외국인·1인세대 요인에 대응 수단이 없다는 빈칸이 여기서 드러나, 다국어 안내와 전입 시점 안내 등 신규 대책 세 가지가 나왔습니다.",
  },
  {
    name: "검증 하네스와 한계 공개",
    easy: "결론을 내기 전에 통계의 전제 조건이 실제로 성립하는지 따로 검사하고, 어긋난 것은 숨기지 않고 적었습니다.",
    here: "잔차 정규성·등분산성·공간 독립성 위배를 확인해 보정 모형을 함께 돌렸고, 청년·외국인·1인세대·무관리주거가 상관 0.85~0.97로 얽혀 있어 무엇이 진짜 요인인지 갈라낼 수 없다는 한계를 밝혀 두었습니다. 어느 하나를 범인으로 지목하는 해석은 삼가 주세요.",
  },
]
      ...
      {section === "data" ? (
        <div className="flex flex-col gap-3">
          ...
          <p className="text-[12.5px] leading-relaxed text-[var(--cp-text-faint)]">
            ...
            verify.py로 해시 대조와 핵심 수치 재계산을 언제든 확인하실 수 있습니다.
          </p>
        </div>
      ) : (
        <div className="flex flex-col gap-3">
