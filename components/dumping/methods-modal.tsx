...
const methods = (data: DumpingMapData, graph: OntoGraph | null): Method[] => {
  ...
  return [
    ...
    },
    {
      ...
      here: `매달 그 이전 자료로만 모수를 고르고 다음 달을 맞히는 롤링 원점 검증(${fc.window})에서 평균 오차 ${fc.mapePct}%, 전년 동월로 찍는 기준모형은 ${fc.naiveMapePct ?? "미산출"}%였습니다. 80% 예측구간 적중률은 ${fc.coverage80Pct ?? "미산출"}%입니다. 인력과 순찰 배치를 위한 행정수요 전망으로만 씁니다.`,
    ...
    },
    {
      ...
      here: `잔차 정규성·등분산성·공간 독립성 위배를 확인해 보정 모형을 함께 돌렸고, 청년·외국인·1인세대·다가구·단독 밀집이 상관 ${col}로 얽혀 있어 무엇이 진짜 요인인지 갈라낼 수 없다는 한계를 밝혀 두었습니다. 어느 하나를 원인으로 지목하는 해석은 피해야 합니다.`,
    },
  ]
}
