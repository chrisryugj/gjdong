...
// 개입수단(Lever) 파생 로직 — 정책 보드 카드와 제안이유 모달이 함께 쓴다.
// 별도 데이터 없이 graph.json의 노드·엣지에서 전부 만들어낸다.
...
// ─── 제안이유 자동 조합 ────────────────────────────────────────
// rationale이 있는 레버는 2건뿐이라, 겨냥 요인의 강도·순위에서 쉬운 문장을 만든다.
...
export function reasonSentences(lv: LeverView, stats: FactorStat[]): string[] {
  ...
  if (!top) {
    return [
      "특정 요인을 겨냥하기보다, 수거·단속 운영 방식 자체를 조정하는 수단이다.",
      "새 예산 없이 지금 있는 인력·노선만 바꿔 시도할 수 있어 먼저 검토할 값어치가 있다.",
    ]
  }
  const s1 = `${top.easy}${josa(top.easy, "을", "를")} 겨냥하는 사업이다.`
  ...
  if (notStat) {
    return [
      s1,
      "다만 근거는 통계로 확인된 효과가 아니다. 이미 갖고 있는 자원을, 무단투기가 한 번도 없던 자리에서 실제로 잦은 자리로 옮기자는 배분 논리다.",
      "돈이 들지 않으니 효과 판정을 기다리지 않고 지금 바로 조정할 수 있다.",
    ]
  }
  ...
  const chance =
    top.p == null || Number.isNaN(top.p) || top.p < 0.001
      ? "우연으로 이런 결과가 나올 확률은 0.1%도 되지 않는다."
      : `우연으로 이런 결과가 나올 확률은 ${(top.p * 100).toFixed(1)}%다.`
  const s2 =
    top.kind === "beta"
      ? `광진구를 100m 격자 ${(top.n ?? 0).toLocaleString()}칸으로 쪼개 견줘 보니, 무단투기가 어디서 생기는지를 ${ordinal(rank)} 잘 설명하는 조건이었다. ${chance}`
      : `행정동 15곳을 나란히 놓고 보면 이 비율이 높은 동네일수록 무단투기도 ${
          top.value >= 0.8 ? "거의 예외 없이" : top.value >= 0.7 ? "뚜렷하게" : "어느 정도"
        } 많았다. 두 값이 함께 움직이는 정도는 ${top.value.toFixed(2)} — 1에 가까울수록 붙어 다닌다는 뜻이다.`
  const s3 = "그런데 지금까지 광진구 대책은 시설과 단속에 몰려 있었고, 이 조건을 직접 건드리는 수단은 비어 있었다."
  return [s1, s2, s3]
}

// 판정 근거(note)가 비어 있는 기존 수단을 위한 기본 설명
export const STATUS_FALLBACK: Record<string, string> = {
  "미검증": "아직 효과를 재 보지 않았다. 설치 시점과 위치 자료를 갖추면 다른 수단과 같은 방식으로 판정할 수 있다.",
  "효과없음": "설치 위치와 무단투기 발생 사이에 이렇다 할 관계가 나타나지 않았다.",
}
