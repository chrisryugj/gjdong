...
const HI = "var(--dump-accent)" // 이 제안이 겨냥하는 요인
...
function StatRow({ s, max, highlight, noun }: { s: FactorStat; max: number; highlight: boolean; noun: string }) {
  ...
  return (
    <div className={`rounded-lg px-2 py-1.5 ${highlight ? "bg-(--dump-accent)/8" : ""}`}>
      <div className="flex items-baseline gap-2">
        <span
          className={`min-w-0 flex-1 text-[15px] leading-snug ${
            highlight ? "font-bold text-(--dump-accent-ink)" : "text-[var(--cp-text-muted)]"
          }`}
        >
          {s.easy}
          {highlight && (
            <span className="ml-1.5 whitespace-nowrap rounded bg-(--dump-accent) px-1.5 py-0.5 text-[12.5px] font-bold text-white">
              이 {noun}이 겨냥
            </span>
          )}
        </span>
...
export default function LeverModal({ lever, graph, onClose, onShowMap }: LeverModalProps) {
  ...
  return (
    ...
    >
          ...
          <div className="mb-4 flex flex-col gap-2">
            {reasons.map((p, i) => (
              <p
                ...
                className={
                  i === 0
                    ? "text-[17px] font-bold leading-[1.6] text-(--dump-accent-ink)"
      ...
      {proposal && (
        <div className="mb-3 rounded-lg bg-(--dump-accent)/8 px-3 py-2.5">
          <p className="flex flex-wrap items-center gap-2 text-[14px] font-bold text-(--dump-accent-ink)">
            가정한 작동 원리
            {lever.mechanism && (
              <span className="rounded-full border border-(--dump-accent)/40 px-2 py-0.5 text-[12.5px] font-semibold">{lever.mechanism}</span>
            )}
          </p>
            ...
            <dl className="mt-1.5 flex flex-col gap-1 text-[15px] leading-relaxed text-[var(--cp-text)]">
              {(
                ...
                <div key={k} className="flex gap-2.5">
                  <dt className="w-8 shrink-0 font-semibold text-(--dump-accent-ink)">{k}</dt>
          ...
          {targetNote(lever) && <p className="mt-1.5 text-[14px] leading-snug text-(--dump-accent-ink)">겨냥 · {targetNote(lever)}</p>}
        </div>
      )}
