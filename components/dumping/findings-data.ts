...
// 핵심 발견 14장. 문장은 여기, 숫자는 map.json·graph.json에서 파생한다.
...
export function buildFindings(data: DumpingMapData, graph: OntoGraph): Finding[] {
  ...
  const findings: Finding[] = [
    ...
    },
    {
      ...
      detail: [
        `공동주택(아파트) 세대수는 무단투기 발생과의 연관을 통계적으로 확인하지 못했습니다(β ${apt ? signed(apt.beta) : "−0.011"}, p=${apt ? pText(apt.p) : "0.708"}). 연관이 없다는 증명은 아닙니다. 같은 수의 사람이 살아도 관리사무소와 경비, 공동 배출장이 있는 주거에서는 발생이 늘지 않았습니다.`,
    ...
    },
    {
      ...
      detail: [
        "\"사람 눈을 피해 으슥한 골목에 버릴 것\"이라는 은폐 가설을 검정해 보니 계수가 둘 다 음수였습니다. 골목이 많은 격자일수록 오히려 발생이 적었고 간선도로에서 멀수록 마찬가지였습니다.",
        "해석: 무단투기는 숨어서 하는 행위가 아니라 사람이 오가는 생활동선 위에서 배출 관리가 없는 곳에 벌어집니다. CCTV를 으슥한 곳에 두는 배치 논리는 데이터와 어긋납니다.",
    ...
    },
    {
      ...
      detail: [
        ...
        `해석: 과태료 부과는 같은 기준으로 ${fmtRatio(g.fines)}, 오히려 ${finesDirection(g)}습니다. 다만 과태료의 ${100 - g.patrolSharePct}%는 신고를 받아 나간 것이라 신고 성향과 무관하지 않습니다. 신고와 독립인 순찰(수시) 적발만 따로 봐도 ${fmtRatio(g.finesPatrol)}로 줄었습니다. 늘어난 부분은 발생 증가보다 신고 채널의 변화로 설명되는 몫이 큽니다. 앱 이용자 수·중복 신고 자료가 없어 발생 증가를 완전히 배제하지는 못합니다. 연도별 민원 건수로 성과를 평가하면 안 되는 이유입니다.`,
    ...
    },
    {
      ...
      detail: [
        ...
        `비교 대상에도 같은 조건을 걸자 그쪽도 0.98건 줄었습니다. 감소분이 평균회귀로 설명되는 셈이라 선택 규칙과 대조군 정의에 민감한 효과 주장은 철회했습니다. 조건을 맞춘 DID는 ${signed(didSym)}(모든 검정 p${didP})이었습니다. 이벤트 스터디(처치 77·대조 667·관측 22,247행)에서도 설치 후 어느 시점 하나 유의하지 않았습니다.`,
    ...
    },
    {
      ...
      body: `최근성을 가중한 점수 상위 20개 격자를 지난 ${bt.windows.length}개 분기로 검증해 보면 평균 ${bt.avgPrecision20 ?? "미산출"}%에서 다음 분기에 실제로 발생했습니다. 무작위보다 ${bt.avgCapture20 && bt.avgRandomCapture ? Math.round(bt.avgCapture20 / bt.avgRandomCapture) : "·"}배 높은 포착률입니다.`,
      detail: [
      ...
      numbers: [
        ...
        { k: "포착률", v: `${bt.avgCapture20 ?? "미산출"}% vs 무작위 ${bt.avgRandomCapture ?? "미산출"}%` },
      ],
  ...
  if (r2) {
    ...
    if (px) {
      ...
      findings.push({
        ...
        body: `건축물대장 "공동주택" ${n(cc.aptHhTotal)}세대 가운데 관리주체가 실제로 있는 K-apt 등록 단지는 ${n(cc.managedTotal)}세대(${Math.round((cc.managedShareOfAptHh ?? 0) * 100)}%)뿐이었습니다. 세 갈래로 나눠 다시 돌리면 다가구·일반단독 β ${signed(sp.unmanaged_units.beta)}는 그대로이고 관리사무소가 없는 다세대·연립은 β ${signed(sp.apt_nokapt.beta)}(p=${pText(sp.apt_nokapt.p)})로 연관이 없습니다.`,
        detail: [
  ...
  if (pm) {
    ...
    findings.push({
      ...
      detail: [
        ...
        "이 소형 주택들은 공동주택관리법 의무관리 기준(150세대)에 못 미쳐 관리사무소와 경비, 공동 배출장이 없는 경우가 많습니다. 다만 대리변수 검증 카드에서 K-apt 미등록 공동주택(다세대·연립·소형)은 과태료와 연관이 없었으므로 이 물량이 곧 발생 증가를 뜻하지는 않습니다. 겨냥점에 직접 닿는 물량은 단독·다가구 허가입니다.",
