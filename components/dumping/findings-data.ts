  ...
  const findings: Finding[] = [
    ...
    },
    {
      ...
      detail: [
        ...
        ...(step
          ? [
              `앱 신고는 서서히 늘지 않았습니다. ${ym(step.month)}에 한 달 만에 ${n(step.from)}건에서 ${n(step.to)}건으로 ${step.ratio.toFixed(1)}배 뛰었고 그 뒤로 그 수준이 이어집니다${step.seoulRatio ? `(서울 전체 청소 신고는 같은 달 ${step.seoulRatio.toFixed(2)}배)` : ""}. 청소과 현장 설명은 이사철·해빙기에 눈에 띄는 투기가 늘어 신고가 몰린다는 것이고, 해마다 2월에서 3월로 신고가 느는 것은 자료와 맞습니다. 다만 그해 배율이 유독 크고 그 수준이 이어진 이유(홍보·포상·앱 연계)는 아직 확인하지 못했습니다. "앱 보급 효과"는 채널별 관측 증가분을 나눈 결과이지, 앱이 원인이라고 식별한 것이 아닙니다.`,
            ]
          : []),
