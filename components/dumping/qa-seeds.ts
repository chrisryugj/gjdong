  ...
  return [
    ...
    },
    {
      ...
      detail: lines(
        ...
        `수치: 재배치 후보 ${candidates}곳(지도 검은 번호), 이동식 CCTV 장부 ${n(cctvRows)}대`,
        `근거: CCTV 현황·과태료 부과 내역, 카드 "효과 철회"`,
        `한계: 재배치도 조치 대장 등록 뒤 평가. 이전·설치 인력 별도`,
      ),
      chart: "did",
      viz: { mode: "enf", layers: ["cctvMobile"], candidates: true },
      vizNote: `지도에 이동식 CCTV 현 위치(보라 점)와 재배치 후보 ${candidates}곳(검은 번호)을 표시했습니다. 지도 오른쪽 목록에서 후보지 주소를 볼 수 있습니다.`,
    },
    {
      q: "빠뜨린 대책은 없나?",
      core: true,
