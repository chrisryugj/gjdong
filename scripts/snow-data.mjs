...
function buildGraph(map) {
  ...
  N("lev-heat", "Lever", "lever", "도로열선", { unit: "구간", count: map.heat.length, length_m: map.heat.reduce((s, h) => s + h.m, 0), phys_m: heatPhys, kind: "상시 설비", def: "급경사·통학로 노면 아래 발열선. 강설 전 자동 가동으로 결빙을 줄입니다" })
  N("lev-salt", "Lever", "lever", "제설함", { unit: "개소", count: map.salt.length, kind: "비치 자재", def: "간선도로변 제설제 보관함. 운전자·관리기관이 즉시 살포합니다" })
  N("lev-cacl", "Lever", "lever", "염화칼슘보관함", { unit: "개소", count: map.cacl.length, kind: "비치 자재", def: "이면도로·주택가 염화칼슘 보관함. 동주민센터가 관리하고 주민이 자율 사용합니다" })
  N("lev-sand", "Lever", "lever", "모래주머니", { unit: "지점", count: map.sand.length, bags: map.sand.reduce((s, x) => s + x.qty, 0), kind: "비치 자재", def: "취약지역·동주민센터 비치 모래. 결빙 노면 미끄럼을 줄입니다" })
  N("lev-sprayer", "Lever", "lever", "자동원격액상살포기", { unit: "대", count: map.ops.sprayers, kind: "상시 설비", def: "급경사 구간 원격 염수 분사. 강설 시 열선과 함께 가동합니다", source: map.ops.source })
  ...
  N("lev-owner", "Lever", "lever", "건축물관리자 자율 제설", { unit: "의무", kind: "법정 의무", def: "조례 제4조 범위(보도 접한 구간·대지경계 1m)를 제5조 시한 안에 치웁니다" })
  ...
  N("kpi-ice-incident", "KPI", "outcome", "결빙 교통사고", { def: "결빙 교통사고 다발지역(반경 200m 3건 이상) 수", measurable: map.accidents ? `다발지역 0곳(${accLast}, 도로교통공단)` : "데이터 없음" })
  ...
  E("kpi-coverage", "operationalizes", "kpi-complaint")
  ...
  CL("claim-update-gap", `구 공개 열선 한 시즌 지연`, `구가 공개한 열선 목록(2025.1, 41구간)은 서울시 집계(2026.5, ${map.heat.length}구간)보다 ${map.heat.length - map.meta.heatJoin.guRows}구간 적습니다. 지도는 서울시 집계를 정본으로 씁니다.`)
  ...
  CL("claim-kpi-missing", `시한 준수·민원은 자료 없음`, `조례 시한 준수와 제설 민원은 공개 데이터가 없습니다. 결빙 교통사고는 다발지역 0곳으로 확인됩니다.`)
