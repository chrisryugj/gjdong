#!/usr/bin/env node
// /dumping 격자 데이터 암호화 — 공개 레포에는 암호문(.enc)만 두고, 빌드가 DUMPING_DATA_KEY로 푼다.
//
...
//     bin-recos.json도 같은 이유로 암호문만 커밋한다(배치 추천 50지점의 대표주소).
// 사용:
...
const FILES = ["map.json", "bin-recos.json"] // 평문은 gitignore, .enc만 커밋
...
function encrypt() {
  ...
  for (const name of FILES) encryptOne(key, resolve(DIR, name), resolve(DIR, `${name}.enc`))
}

function encryptOne(key, PLAIN, ENC) {
...
function decrypt() {
  const key = loadKey()
  for (const name of FILES) decryptOne(key, name, resolve(DIR, name), resolve(DIR, `${name}.enc`))
}

function decryptOne(key, name, PLAIN, ENC) {
  if (!existsSync(ENC)) {
    if (existsSync(PLAIN)) return console.log(`decrypt: ${name}.enc 없음, 평문 사용`)
    throw new Error(`${name}.enc도 ${name}도 없습니다`)
  }
  if (!key) {
    if (existsSync(PLAIN)) return console.log(`decrypt: 키 없음, 기존 ${name} 평문 사용`)
    throw new Error(`DUMPING_DATA_KEY 없음 — 빌드 환경(Vercel env)에 키를 넣어야 ${name}을 풀 수 있습니다`)
  }
  const buf = readFileSync(ENC)
  if (buf.subarray(0, 7).toString() !== MAGIC) throw new Error(`${name}.enc 포맷이 다릅니다`)
