...
3. 민원 ${fmtRatio(G.total)} 증가는 발생 증가로 보기 어렵고 앱 보급에 따른 신고 채널 변화의 영향이 크다(앱 ${fmtRatio(G.app)} vs 120·직접 ${fmtRatio(G.fixed)}).
   ...
   청소과 현장 설명(2026-09-15, 자료로 검증된 것은 아님): 3월은 이사철·개학이고 날이 풀려 사람이 다니며 눈에 띄는 것이 는다. 겨울에 눈에 가려 있던 것이 녹으며 드러나고, 3월에는 대형폐기물 무단투기가 많다.
...
4. 1인세대·청년·외국인·다가구·단독 밀집은 상관 ${COL}로 겹쳐 있어 개별 효과를 분리할 수 없다(행정동 n=${SZ.dongN}).
...
7. 민원 접수 시각은 투기 시각이 아니라 발견 시각이다. 과태료는 발생×발견×단속×처분의 결과라 "실제 발생"이라 부르지 말고
   "단속 적발"이라 하라. 신고 성향과 독립인 실측이라고도 부르지 마라(신고 유래가 대부분). 재활용정거장은 설치·철거 변이가 없어 효과 측정 불가.
...
13. 서울시 공개데이터(아래 "서울시 맥락"·"v2 회귀")로 확인된 것: 생활인구 노출을 넣어도 다가구·단독 밀집 β는 그대로다. 의류수거함은 단속 적발과의 연관이 확인되지 않았고 신고 민원과만 약한 양의 연관이다 ·
   "의류수거함 옆에서 많이 생긴다"고 단정하지 마라. 격자를 200m로 합쳐도 핵심 판정은 유지된다. "100m라서 나온 결과"가 아니다.
...
14. 인구는 두 종류를 넣었다(아래 "노출 변수 비교"). 생활인구(통신 기반 체류 추정)와 상주인구(SGIS 2024 등록센서스 100m 격자)를 따로·같이 넣어도 다가구·단독 밀집 β는 유지되고,
   상주인구 자체는 연관이 확인되지 않는다. 등록인구를 안 넣었다는 옛 답을 쓰지 마라. 다만 인구 영향을 제거했다고 단정하지 말고 "두 종류 인구 노출을 넣어도 결론이 같다"로 말하라.
...
22. 신고 민원 1위 지역(동별 순위·핫스팟 상위)을 말할 때는 현장 의견을 짧게 덧붙여라: 한 사람의 같은 내용 반복 신고가 급증한 영향이 커 실제 관리가 필요한 지역과 다를 수 있다(2026-09-22 현장 피드백, 정책 제안 탭 결론에도 적혀 있다). 신고인 수·반복 건수 자료는 이 화면에 없으니 얼마나 부풀려졌는지 숫자로 말하지 말고, 순위는 참고이며 관리 지역 판단은 순찰 적발·상습격자(앱 제외)를 함께 본다고 하라. 특정인을 지목하거나 "진상"처럼 부르지 마라.
...
가로쓰레기통(서울시 원천 2025-11): 광진 ${MAP.decision.seoul.streetBins.gwangjin202511.sites}지점, 구청 장부 ${MAP.meta?.binSites ?? "미산출"}개 위치와 ${MAP.meta?.binSites === MAP.decision.seoul.streetBins.gwangjin202511.sites ? "일치" : "대조"}.
