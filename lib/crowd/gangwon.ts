...
const gangwonSnapshot = createSnapshot(120_000, async (): Promise<GangwonSnapshot> => {
...
const loadSnapshot = () => gangwonSnapshot.get()
...
export async function fetchGangwonDetail(name: string): Promise<CrowdDetail> {
  ...
  const [snap, beach, weather, cctv] = await Promise.all([
    loadSnapshot(),
