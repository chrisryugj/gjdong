  ...
  class DeckStage extends HTMLElement {
    ...
    _render() {
      ...
      overlay.innerHTML = `
        ...
        <button class="btn fullscreen" type="button" aria-label="Toggle fullscreen" title="Fullscreen (F)">
          <svg class="ic-enter" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 6V3h3M13 6V3h-3M3 10v3h3M13 10v3h-3"/></svg>
          <svg class="ic-exit" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" style="display:none;"><path d="M6 3v3H3M10 3v3h3M6 13v-3H3M10 13v-3h3"/></svg>
        </button>
        <span class="divider"></span>
      ...
      this._fsIconEnter = this._fsBtn.querySelector('.ic-enter');
      this._fsIconExit = this._fsBtn.querySelector('.ic-exit');
    }
    ...
    _toggleFullscreen() {
      ...
      if (fsEl) {
        ...
        if (req) {
          try { req.call(this); } catch (e) { console.warn('[deck-stage] fullscreen failed:', e); }
        }
      }
    }
    ...
    _onFullscreenChange() {
      ...
      if (this._fsIconEnter) this._fsIconEnter.style.display = isFs ? 'none' : 'block';
      if (this._fsIconExit) this._fsIconExit.style.display = isFs ? 'block' : 'none';
      this._flashOverlay();
    }
    ...
    _loadNotes() {
      ...
      if (tag) {
        try {
          ...
          if (Array.isArray(parsed) && parsed.length) this._notes = parsed;
        } catch (e) {
    ...
    _applyIndex({ showOverlay = true, broadcast = true, reason = 'init' } = {}) {
      ...
      // Video lifecycle: rewind+play on entry, pause on exit.
      ...
      if (this._slides[curr]) {
        this._slides[curr].querySelectorAll('video').forEach((v) => {
          try {
            v.currentTime = 0;
            const p = v.play();
    ...
    _onKey(e) {
      ...
      const key = e.key;
      const code = e.code; // physical key — IME/한글 모드 우회용
      let handled = true;
