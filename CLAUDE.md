...
7. **클린광진 상황실** (`/dumping`, 암호 게이트 `DUMPING_PASSWORD`) — 광진구 무단투기 발생구조 분석 대시보드. 데이터·분석 SSOT는 **비공개 저장소 chrisryugj/gwangjin-dumping**(로컬 `~/workspace/gwangjin-dumping`, 원자료에 건별 민원·과태료 기록이 있어 공개 레포에 절대 커밋 금지) (README가 정본: 회귀·DID 철회·하네스·REPRODUCE 해시). 파이프라인: `build_decision_layer.py`(품목·처분퍼널·SLA·상습격자 KPI·핫스팟 백테스트·홀트윈터스 수요전망) + `fetch_permits.py`(건축HUB 인허가 구조전망, ARCHHUB_SERVICE_KEY 필요·최초본은 MCP 실측) → `export_dashboard.py`가 `public/dumping/{map,graph}.json` 생성. ⚠️ontology.db는 REPRODUCE 해시 대상이라 불변 — CCTV 철회·rho 재표기·의사결정 노드(DECISION_NODES) 전부 export 시점 주석 레이어. 스크립트 수정 시 `REPRODUCE/manifest.json` 해시 갱신 + `verify.py` 통과 필수. UI: [components/dumping/](components/dumping/) 탭 4개, 기본=물어보기(qa-chat: 지도앱식 검색바 + 핵심 질의응답 9 아코디언(상위 3 펼침), 답변 최신순·지도 반영은 명시 버튼)·발견 10카드·운영·전망(ops-panel: KPI/핫스팟20/전망차트/품목/퍼널/SLA/구조전망/조치대장)·정책(onto-panel: 정책 지도(policy-board — graph.json Lever/KPI 파생, 제안 6·기존 판정 4·성과지표·사전등록 원칙)/전체 탐색). 운영 탭 인터랙션: 지도 가능한 건 지도로(핫스팟 20 순위배지 레이어 `showHotspots`=ops탭 상시, 상습격자 33 격자강조 `showCritical`=KPI 타일 토글, 목록 클릭=펄스 하이라이트+라벨 `CandidateFocus.label`), 나머지는 중앙 상세모달(ops-modal.tsx: funnel/channels/forecast/fines/sla/permits — ModalShell·KRW·ForecastChart 공용 export). 헤더 "데이터·방법" = methods-modal(쓰인 데이터: 구청 제공 6종/직접 수집 7종 구분 + 8개 방법론 일반인 해설). 동 브리핑 인쇄 = briefing-modal + globals.css `#dump-brief` print 규칙. 조치 대장 = `public/dumping/interventions.json`(개입 사전등록부, registeredAt 빈 항목은 예시로 UI 제외). QA 컨텍스트 = [lib/dumping/context.ts](lib/dumping/context.ts) (인과표현·what-if 금지 등 해석 규칙 11개 — 수치 갱신 시 여기도 동기화). 미확보 데이터 명세 = gwangjin-dumping/docs/NEEDED_DATA.md. ⚠️프로덕션 `next start`는 시작 후 추가된 public 파일을 404로 냄 — 새 public 파일은 서버 재시작.
