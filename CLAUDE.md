...
5. **Seoul Crowd Radar** (`/crowd`) - Real-time congestion dashboard for 121 Seoul hotspots via the Seoul RTD open API proxy ([lib/crowd/seoul-rtd.ts](lib/crowd/seoul-rtd.ts), Referer header required). GPS nearby recommendations, purpose presets, favorites (localStorage), `?spot=` deep links, Kakao/Naver directions, PWA install (`public/crowd-manifest.webmanifest` + `crowd-sw.js`), and a day×hour heatmap: `.github/workflows/crowd-heatmap.yml` collects every 3h via `scripts/collect-crowd-heatmap.mjs` (ppltn_congest 12h lookback of observed levels, global `lastSlot` dedup marker — self-heals missed cron runs) into the orphan `data` branch (`heatmap.json`), which the client reads from raw.githubusercontent.com. Spot detail panels lazy-load extras (accidents/parking/events/road/bike) from `/api/crowd/extra?spot=`, and the spot list response carries today's disaster messages for the dashboard banner. Fully localized in ko/en/ja/zh ([lib/crowd/i18n.ts](lib/crowd/i18n.ts) dictionary incl. all 121 spot names, [components/crowd/lang-context.tsx](components/crowd/lang-context.tsx) provider + flag switcher): auto-detects browser language, persists to `localStorage`, `?lang=` deep links get localized metadata via `generateMetadata`; API queries always use Korean spot names as keys.
