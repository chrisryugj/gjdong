...
// 하루 축 묶음 — 행사·EV·무더위쉼터·지하철역 좌표·공공예약. 갱신이 느린 원천이라 길게 캐시
const CACHE_HEADERS = { "Cache-Control": "public, s-maxage=1800, stale-while-revalidate=3600" }

export async function GET() {
  const [events, ev, shelters, stations, reservations] = await Promise.all([
    fetchEvents(),
    fetchEv(),
    fetchShelters(),
  ...
  return NextResponse.json({ events, ev, shelters, stations, reservations }, { headers: CACHE_HEADERS })
}
